...
export default defineAction({
  ...
  schema: z.object({
    ...
    workOrderId: z.string().uuid().optional().describe("Factory WorkOrder ID to open"),
    activeTab: z.enum(["overview", "activity", "changes", "verification", "decisions"]).optional().describe("Factory detail section"),
    selectedEvidenceId: z.string().optional().describe("Factory event, check, or candidate evidence ID"),
  }),
  http: false,
  run: async (args) => {
    ...
    if (args.workOrderId) nav.workOrderId = args.workOrderId;
    if (args.activeTab) nav.activeTab = args.activeTab;
    if (args.selectedEvidenceId) nav.selectedEvidenceId = args.selectedEvidenceId;
