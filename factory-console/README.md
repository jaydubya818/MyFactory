...
Open the local URL shown by the development server. The root page opens `/factory`. Select a WorkOrder to inspect activity, candidate changes, verification, and decisions. The agent receives the current view, WorkOrder, tab, and selected evidence through `view-screen`. Its write tools can add a note or request publication; neither action approves or publishes a draft PR.
